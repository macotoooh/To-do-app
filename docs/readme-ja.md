...
Todo詳細・編集画面では、入力中のタイトルをもとに  
AIが関連するタスク案を自動で提案します。

- ワンクリックでAIによるタスク案を生成
- 現在のタスクに関連した「次にやること」を提案
- 追加したい案を選択して保存すると、新しいTodoとして登録
- AIタスクの追加に成功した場合は、分かりやすいトーストでフィードバックを表示

この機能により、タスクの分解や次のアクションを考える負担を軽減します。

<img src="./images/ai-thinking.png" width="500" />
<img src="./images/ai-suggestions.png" width="500" />

AI機能は React Router の action を通じて実装しており、
OpenAI API はサーバー側のみで呼び出しています。
APIキーがクライアントに露出しない安全な構成です。
...
このアプリケーションでは、**エラーが発生する文脈（データ取得か、ユーザー操作か）に応じて
異なるエラーハンドリング戦略**を採用しています。

### Loader のエラー（データ取得時）

`loader` はデータ取得時のエラーを **HTTP エラーとして扱います**。

- リソースが存在しない場合は `404 Not Found`
- 想定外のエラーやシステムエラーは `500 Internal Server Error`

意図的に投げられた `Response`（404 など）はそのまま維持し、
予期しないエラーは共通ユーティリティを使って
`500 Internal Server Error` に正規化しています。

### Action のエラー（ユーザー操作時）

作成・更新・削除などの **ユーザー操作に起因するエラー**は、
`Response` を throw せず、**構造化されたデータとして UI に返します**。

これにより：

- ページ遷移を発生させない
- フォームの入力状態を保持できる
- トーストなどで文脈に沿ったフィードバックを表示できる

といった UX を実現しています。

### Error Boundary（エラー画面の表示）

- 詳細ページでは、ルート単位の Error Boundary を使用し、
  loader から返された HTTP ステータス（404 / 500）に応じて
  専用のエラー画面を表示します
